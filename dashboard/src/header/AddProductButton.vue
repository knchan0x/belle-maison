...
const imageUrl = computed(() => {
    ...
    return pageState.data?.Product.Styles.find(e => e.StyleCode === pageState.selectedStyleCode)?.ImageUrl;
});
...
const price = computed(() => {
    ...
    return pageState.data?.Product.Styles.find(e => e.StyleCode === pageState.selectedStyleCode)?.Price;
});
...
const loadProductInfo = async (productCode: string) => {
    ...
    try {
        ...
        pageState.errMsg = '';
        pageState.options = stylesToOptions(response.Product.Styles.sort((a, b) => parseInt(a.StyleCode) - parseInt(b.StyleCode)) || []);
    } catch(e: any) {
...
const handleOk = async () => {
    ...
    const colour = pageState.data?.Product.Styles.find(e => e.StyleCode === pageState.selectedStyleCode)?.Colour || '';
    const size = pageState.data?.Product.Styles.find(e => e.StyleCode === pageState.selectedStyleCode)?.Size || '';

    try {
        await targets.add(pageState.productCode, colour, size, pageState.targetPrice);
    ...
    <div>
        ...
        <a-modal v-model:visible="pageState.isVisible" title="Add New Product" @ok="handleOk" :closable="false"
            ...
            <template #footer>
                ...
                <a-button key="Add" type="primary" :disabled="lockAdd" :loading="pageState.isConfirming"
                    @click="handleOk">Add
                </a-button>
            </template>
                ...
                <a-form-item label="Product URL" name="url">
                    <a-row justify="space-around">
                        <a-col :span="20">
                            ...
                            <p v-if="pageState.warning !== ''" style="color:red;">{{ pageState.warning }}</p>
                        </a-col>
                        <a-col :span="4">
                    ...
                    <div v-else>
                        <div v-if="!pageState.isError">
                            <product-info @selected="(selected: string) => pageState.selectedStyleCode = selected[1]"
                                @target="(price: number) => pageState.targetPrice = price"
                                :style-options='pageState.options' :price="price" :image-url="imageUrl" />
                        </div>
                        <div v-else>
