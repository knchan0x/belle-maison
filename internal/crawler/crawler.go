...
var (
	ErrMultipleClient = errors.New("more than one http client assigned")
)
...
type Crawler interface {
	RetrieveProduct(productCode string) *Result
	RetrieveProducts(productCodes ...string) []*Result
}
...
// RetrieveProducts retrieves multiple products' info from the site
func (c *crawler) RetrieveProducts(productCodes ...string) []*Result {
	...
	go func(respCh <-chan *response, resultCh chan<- *Result) {
		...
		for resp := range respCh {
			...
			go func(resp response, resultCh chan<- *Result) {
				...
				resultCh <- &result
			}(*resp, resultCh) // copy data instead of pointer
		}
		wg.Wait()
		close(resultCh)
	}(respCh, resultCh)
	...
	for _, id := range productCodes {
		...
		data, err := retrieveData(c.httpClient, baseURL+id)
		if err != nil {
			resp.err = err
		} else {
			resp.data = data
...
// RetrieveProduct retrieves product info from the site
func (c *crawler) RetrieveProduct(productCode string) *Result {
	data, err := retrieveData(c.httpClient, baseURL+productCode)
	if err != nil {
		return &Result{
...
// retrieveData retrieves web page from the site
func retrieveData(httpClient HTTPClient, link string) ([]byte, error) {
	// Set header
...
// parseHTML parses the html page to Product struct
